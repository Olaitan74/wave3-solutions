...
win_count_dict = {}
for year, dir_name in winners.items():
    for winner in dir_name:
        if winner in win_count_dict:
            win_count_dict[winner] += 1
        else:
            win_count_dict[winner] = 1

highest_oscar_win_count = 0

for year, dir_win in win_count_dict.items():
    if dir_win > highest_oscar_win_count:
        highest_oscar_win_count = dir_win
        most_win_director.clear()
        most_win_director.append(year)
    elif dir_win == highest_oscar_win_count:
        most_win_director.append(year)
    else:
        continue
